...
const appHandle: Handle = async ({ event, resolve }) => {
	...
	if (!event.locals.user && !event.url.pathname.startsWith('/auth')) {
		const calorieURL = publicEnv.PUBLIC_CALORIE_URL ?? `${event.url.protocol}//${event.url.host}`;
		const redirectTo = encodeURIComponent(`${calorieURL}${event.url.pathname}${event.url.search}`);
	...
	if (event.locals.user && !event.url.pathname.startsWith('/auth')) {
		...
		const access = await withUser(userId, (tx) =>
			tx
				.select()
				.from(userAppAccess)
				.where(and(eq(userAppAccess.userId, userId), eq(userAppAccess.app, 'calorie')))
				.limit(1)
		);

		if (access.length === 0) {
			logger.warn('app access denied', { email: event.locals.user.email, app: 'calorie' });
			redirect(303, `${auth.options.baseURL}/not-authorized-app?app=calorie`);
		}
	}
